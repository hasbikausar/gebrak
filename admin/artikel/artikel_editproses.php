<?php
  ...
  if(move_uploaded_file($lokasi_gambar, $path)){ // Cek apakah gambar berhasil diupload atau tidak
  	...
  	$query = "SELECT * FROM artikel WHERE id_artikel='$_POST[id]'";
    ...
    // Proses ubah data ke Database
		$query = "UPDATE artikel SET judul='$_POST[judul]', isi='$isi', gambar='$nama_gambar' where id_artikel='$_POST[id]'";
		$sql = mysqli_query($db_link, $query); // Eksekusi/ Jalankan query dari variabel $query

		if($sql){ // Cek jika proses simpan ke database sukses atau tidak
			// Jika Sukses, Lakukan :
      echo "Artikel telah diedit";
      echo "<meta http-equiv='refresh' content='1; url=?tampil=artikel'>";

    }else{
			// Jika Gagal, Lakukan :
			echo "Maaf, Terjadi kesalahan saat mencoba untuk menyimpan data ke database.";
			echo "<meta http-equiv='refresh' content='60; url=?tampil=artikel'>";
		}
	}else{
	...
	// Proses ubah data ke Database
	$query = "UPDATE artikel SET judul='$_POST[judul]', isi='$isi' where id_artikel='$_POST[id]'";
	$sql = mysqli_query($db_link, $query); // Eksekusi/ Jalankan query dari variabel $query
